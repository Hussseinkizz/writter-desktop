...
export function detectOS(): OSType {
  ...
  // Handle the safeTry result properly
  if (result instanceof Promise) {
    // This shouldn't happen in this case since we're not using async operations
    return 'unknown';
  }
...
export function getLatestReleaseAssets(): DownloadAsset[] {
  ...
  const baseUrl = 'https://github.com/Hussseinkizz/writter-desktop/releases/download/app-v0.0.9';
...
export function getPrimaryDownload(): DownloadAsset | null {
  ...
  return assets.find(asset => 
    asset.os === userOS && asset.type === preferredTypes[0]
  ) || null;
}
